...
import com.pf.Shopping.dao.CommonDao;
import com.pf.Shopping.pojo.Params;
...
public class CommonDaoImpl<T> {
	...
	*/
	 ...
	 * 
	 * @author 周化益
	 * @param beanClass 查询的表映射的实体类
	 * @param columnName 查询的字段名
	 * @param columnValue 查询的字段对应的值
	 * @return
	 */
	protected Map<String, Object> findById(Class<T> beanClass, String columnName, String columnValue) {
		StringBuffer sqlBuffer = new StringBuffer("SELECT * FROM ");
		sqlBuffer.append(beanClass.getSimpleName());
		sqlBuffer.append(" WHERE ").append(columnName).append(" = ").append(TransactSQLInjection(columnValue));
		return commonDao.getMapClass(sqlBuffer.toString());
	}
